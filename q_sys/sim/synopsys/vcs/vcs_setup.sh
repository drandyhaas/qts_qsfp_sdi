...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [q_sys::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {q_sys::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [q_sys::get_design_files "$QSYS_SIMDIR"]]
