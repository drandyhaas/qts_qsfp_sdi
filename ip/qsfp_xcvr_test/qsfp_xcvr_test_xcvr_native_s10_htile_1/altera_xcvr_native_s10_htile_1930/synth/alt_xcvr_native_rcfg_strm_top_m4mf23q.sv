...
import alt_xcvr_native_rcfg_strm_params_m4mf23q::*;

module alt_xcvr_native_rcfg_strm_top_m4mf23q #(
  ...
  parameter xcvr_rcfg_addr_width  = 11,               //XCVR RCFG address width
  ...
  // User interfce ports
  input  wire [xcvr_rcfg_interfaces*(altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(rcfg_profile_cnt-1))-1:0]  cfg_sel,
  ...
  output wire [(xcvr_rcfg_shared ? (xcvr_rcfg_addr_width+altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(xcvr_rcfg_interfaces-1)) : 
  ...
  localparam cfg_addr_width = altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(get_max_value(rcfg_profile_cnt, rcfg_cfg_depths)-1); //Maximum ROM address width

  wire [cfg_addr_width-1:0]                   cfg_address;
  wire [rom_data_width-1:0]                   rom_readdata;
  reg  [(altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(rcfg_profile_cnt-1))-1:0]     r_cfg_sel   = {(altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(rcfg_profile_cnt-1)){1'b0}};
  reg                                         r_bcast_en  = 1'b0;
  reg                                         r_cfg_load  = 1'b0;
  reg  [31:0]                                 if_sel      = 32'b0; //Declared as 32-bit as altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10 returns integer. It is appropriately optimized by the synthesis tool.
    ...
    if (reset) begin
      ...
      if(~|stream_busy & |cfg_load & ~if_sel_lock) begin                                // Check if previous reconfig operation is complete and there are any other requests pending
        if((cfg_load & ~if_sel_mask_tmp) == {xcvr_rcfg_interfaces{1'b0}}) begin
          ...
          if_sel <= altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(cfg_load) - {{(xcvr_rcfg_interfaces-1){1'b0}},1'b1}; // Reset if_sel to current cfg_load if this is not one of the pending transactions
        end else begin
          if_sel_mask <= if_sel_mask_tmp;
          if_sel <= altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(cfg_load & ~if_sel_mask_tmp & ((~cfg_load | if_sel_mask_tmp) + 1)) - {{(xcvr_rcfg_interfaces-1){1'b0}},1'b1}; // Select the next request to service based on round-robin scheduling (less significant cfg_load bit = higher priority)
        end
  ...
  // Register the control signals
   always @ (posedge clk or posedge reset) begin
    if (reset) begin
      r_cfg_sel  <= {(altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(rcfg_profile_cnt-1)){1'b0}};
      r_bcast_en <= 1'b0;
      r_cfg_load <= 1'b0;
    end
    else begin
      if(cfg_load[if_sel] & ~stream_busy[if_sel]) begin
        r_cfg_sel  <= cfg_sel[(altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(rcfg_profile_cnt-1))*if_sel +: (altera_xcvr_native_s10_functions_h::clogb2_alt_xcvr_native_s10(rcfg_profile_cnt-1))];
  ...
  alt_xcvr_native_rcfg_strm_rom_m4mf23q #(
  ...
  alt_xcvr_native_rcfg_strm_ctrl_m4mf23q #(
