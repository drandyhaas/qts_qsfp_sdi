...
# ---      alt_xcvr_native_helper_functions_jn2dlsi.tcl  --- #
# -                                                              - # 
# ---------------------------------------------------------------- #

set script_dir [file dirname [info script]] 
source "${script_dir}/qsfp_xcvr_test_xcvr_native_s10_0_altera_xcvr_native_s10_16915_jn2dlsi_ip_parameters_jn2dlsi.tcl"
source "${script_dir}/alt_xcvr_native_helper_functions_jn2dlsi.tcl"

# Debug switch. Change to 1 in alt_xcvr_native_helper_functions_jn2dlsi.tcl to get more run-time debug information
...
# -                                                              - #
# ---------------------------------------------------------------- #
if { ! [ info exists native_sdc_cache_jn2dlsi ] && !$native_debug } {
   native_initialize_db_jn2dlsi nativedb_jn2dlsi
   set native_sdc_cache_jn2dlsi 1
} else {
  if { $native_debug } {
    native_initialize_db_jn2dlsi nativedb_jn2dlsi
...
# ---------------------------------------------------------------- #
set alt_xcvr_native_s10_instances [ dict keys $nativedb_jn2dlsi ]
...
foreach inst $alt_xcvr_native_s10_instances {
  ...
  dict set alt_xcvr_native_s10_pins $inst [dict get $nativedb_jn2dlsi $inst]
  ...
  for {set i 0} {$i < $profile_cnt} {incr i} {
    ...
    if {[dict get $native_phy_ip_params rx_enable_profile$i]} {
      ...
      if {[dict get $native_phy_ip_params enable_port_rx_clkout2_profile$i] == 1 && $rx_fifo_mode == "pc_fifo"} {
        ...
        } elseif {$rx_clkout2_sel == "pcs_x2_clkout" && $rx_clkout_sel != "pcs_x2_clkout"} {
      ...
      foreach clk_group $mode_clks { # Each mode can have multiple clocks; iterate over them
        ...
        if {[dict exists $alt_xcvr_native_s10_pins $inst $clk_group]} {
          ...
          if { [llength $clk_pins] > 0 } { # Check to see if the corresponding pins exists 
            ...
            lappend list_of_clk_names [native_create_clocks_all_ch_jn2dlsi $inst $clk_group $channel_number $clk_freq $clk_pins $profile_cnt $i]
          } else {
            if {$native_debug == 1} {
...
#
#        if {$protocol_mode == "pipe_g2" || $protocol_mode == "pipe_g3"} {
#          dict lappend native_phy_pdn_clocks tx_mode_clks tx_coreclkin_dcm_pipe_g1
#          dict lappend native_phy_pdn_clocks tx_mode_clks tx_coreclkin_dcm_pipe_g2
#
#          if { $protocol_mode ==  "pipe_g3"} {
#            dict lappend native_phy_pdn_clocks tx_mode_clks tx_coreclkin_dcm_pipe_g3
#          }
#        } else {
...
#        if {$protocol_mode == "pipe_g2" || $protocol_mode == "pipe_g3"} {
#          dict lappend native_phy_pdn_clocks tx_mode_clks tx_coreclkin_rowclk_pipe_g1
#          dict lappend native_phy_pdn_clocks tx_mode_clks tx_coreclkin_rowclk_pipe_g2
#
#          if { $protocol_mode ==  "pipe_g3"} {
#            dict lappend native_phy_pdn_clocks tx_mode_clks tx_coreclkin_rowclk_pipe_g3
#          }
#
#        } else {
...
#
#          if {$protocol_mode == "pipe_g2" || $protocol_mode == "pipe_g3"} {
#            dict lappend native_phy_pdn_clocks rx_mode_clks rx_coreclkin_dcm_pipe_g1
#            dict lappend native_phy_pdn_clocks rx_mode_clks rx_coreclkin_dcm_pipe_g2
#
#            if { $protocol_mode ==  "pipe_g3"} {
#              dict lappend native_phy_pdn_clocks rx_mode_clks rx_coreclkin_dcm_pipe_g3
#            }
#          } else {
...
#
#        if {[dict get $native_phy_ip_params enable_port_rx_clkout2_profile$i] == 1} {
#          if {$rx_clkout2_sel == "pcs_clkout" || $rx_clkout2_sel == "pma_div_clkout"} {
#
#            if {[dict get $native_phy_ip_params rx_coreclkin_clock_network_profile$i] == "dedicated"} {
#              dict lappend native_phy_pdn_clocks rx_mode_clks rx_coreclkin_dcm
#            } elseif {[dict get $native_phy_ip_params rx_coreclkin_clock_network_profile$i] == "rowclk"} {
#              dict lappend native_phy_pdn_clocks rx_mode_clks rx_coreclkin_rowclk
#            } else {
#              post_message -type error "IP SDC: Clock network for rx_coreclkin was not recognized. Check the RX Clock Options"
#            }
#
...
#            set clk_pins [string map {\\ \\\\} $clk_pins]
#
#            lappend list_of_clk_names [native_create_pdn_clocks_all_ch_jn2dlsi $inst $clk_group $channel_number $clk_pins $profile_cnt $i $protocol_mode]
#          } else  {
#            if {$native_debug == 1} {
#              post_message -type warning "IP SDC Warning: No pins for $clk_group"
#            }
