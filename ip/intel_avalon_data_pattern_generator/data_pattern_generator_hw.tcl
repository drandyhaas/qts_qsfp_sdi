...
# | 
set_module_property DESCRIPTION "Data Pattern Generator"
set_module_property NAME data_pattern_generator 
set_module_property VERSION 1.0
set_module_property INTERNAL false
set_module_property AUTHOR "Intel Corporation"
set_module_property DISPLAY_NAME "Data Pattern Generator"
...
set_parameter_property ST_DATA_W DISPLAY_HINT "Avalon-ST Port Data Width"
...
set_parameter_property BYPASS_ENABLED DISPLAY_HINT "Select to enable bypass interface section"
...
set_parameter_property AVALON_ENABLED DISPLAY_HINT "Select to enable avalon input streaming interface"
...
set_parameter_property FREQ_CNTER_ENABLED IS_HDL_PARAMETER false 
...
# Generator instance 
# +-----------------------------------
add_instance "data_pattern_generator" data_pattern_generator_core
...
proc composition {} {
    ...
    foreach param $checker_parameters {
        set_instance_parameter_value data_pattern_generator $param [get_parameter_value $param]
    }
    ...
    if { $bypass == "true" } {
        if { $avalon == "true" } {
            set avalon_input "true"
            set conduit_input "false"
        } else {
            set avalon_input "false"
            set conduit_input "true"
        }
    } else {
            set avalon_input "false"
            set conduit_input "false"
    ...
    if { $avalon == "true" } {
        add_interface pattern_out_clk clock end
        set_interface_property pattern_out_clk ENABLED $avalon

        add_interface pattern_out avalon_streaming source
        set_interface_property pattern_out ENABLED $avalon
        set_interface_property pattern_out export_of data_pattern_generator.pattern_out
        set_interface_property pattern_out PORT_NAME_MAP {aso_data aso_data aso_valid aso_valid aso_ready aso_ready}

        set_interface_assignment pattern_out debug.controlledBy csr_slave
    } else {
        add_interface conduit_pattern_out_clk conduit end 
        set_interface_property conduit_pattern_out_clk ENABLED $conduit 

        add_interface conduit_pattern_out conduit start 
        set_interface_property conduit_pattern_out ENABLED $conduit
        set_interface_property conduit_pattern_out export_of data_pattern_generator.conduit_pattern_out 
        set_interface_property conduit_pattern_out PORT_NAME_MAP {aso_data aso_data}

        set_interface_assignment conduit_pattern_out debug.controlledBy csr_slave
    }

    if { $avalon_input == "true" } {
        add_interface pattern_in_clk clock source 
        set_interface_property pattern_in_clk ENABLED $avalon_input
        set_interface_property pattern_in_clk export_of data_pattern_generator.pattern_in_clk

        add_interface pattern_in avalon_streaming sink
        set_interface_property pattern_in ENABLED $avalon_input 
        set_interface_property pattern_in export_of data_pattern_generator.pattern_in
        set_interface_property pattern_in PORT_NAME_MAP {asi_data asi_data asi_valid asi_valid asi_ready asi_ready}

    }

    if { $conduit_input == "true"} {
        add_interface conduit_pattern_in_clk conduit end 
        set_interface_property conduit_pattern_in_clk ENABLED $conduit_input
        set_interface_property conduit_pattern_in_clk export_of data_pattern_generator.conduit_pattern_in_clk

        add_interface conduit_pattern_in conduit end 
        set_interface_property conduit_pattern_in ENABLED $conduit_input
        set_interface_property conduit_pattern_in export_of data_pattern_generator.conduit_pattern_in
    ...
    if { $cnter == "true" } {
        ...
        add_connection mm_bridge.m0 data_pattern_generator.csr_slave
        set_connection_parameter_value mm_bridge.m0/data_pattern_generator.csr_slave baseAddress 0x00
        ...
        add_connection  mm_clk_source.clk       data_pattern_generator.csr_clk
        add_connection  mm_clk_source.clk       mm_bridge.clk    

        add_connection  mm_clk_source.clk_reset data_pattern_generator.csr_clk_reset
        ...
        if { $avalon == "true" } {
            ...
            add_instance "pattern_out_clk_source" clock_source
            set_instance_parameter_value pattern_out_clk_source clockFrequencyKnown false
            set_interface_property pattern_out_clk export_of pattern_out_clk_source.clk_in

            add_connection mm_clk_source.clk_reset      pattern_out_clk_source.clk_in_reset 
            add_connection pattern_out_clk_source.clk   data_pattern_generator.pattern_out_clk
            add_connection pattern_out_clk_source.clk   frequency_cnter.avalon_des_clk
         } else {
            add_instance "pattern_out_clk_splitter" conduit_splitter
            set_instance_parameter_value pattern_out_clk_splitter OUTPUT_NUM 2

            set_interface_property conduit_pattern_out_clk export_of pattern_out_clk_splitter.conduit_input
            add_connection pattern_out_clk_splitter.conduit_output_0 data_pattern_generator.conduit_pattern_out_clk
            add_connection pattern_out_clk_splitter.conduit_output_1 frequency_cnter.conduit_des_clk 
         }
    } else {
        # if no frequency cnter is enabled, directly export the interfaces
        set_interface_property csr_slave        export_of   data_pattern_generator.csr_slave
        set_interface_property csr_clk          export_of   data_pattern_generator.csr_clk
        set_interface_property reset            export_of   data_pattern_generator.csr_clk_reset
        if { $avalon == "true" } {
            set_interface_property pattern_out_clk   export_of   data_pattern_generator.pattern_out_clk
        } else {
            set_interface_property conduit_pattern_out_clk export_of data_pattern_generator.conduit_pattern_out_clk
        } 
    }
}
